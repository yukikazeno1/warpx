...
RUN_DIR="${SCRIPT_DIR}/runs_ishizawa_scale_ensemble/seed${SEED_TAG}/nocoll_ppc${PPC}"
INPUT_FILE="${RUN_DIR}/inputs_scale_pilot_seeded"
...
echo "=============================================================================="
echo "Ishizawa-scale SEEDED collisionless run"
echo "seed                  : ${SEED}"
echo "mode                  : ${MODE}"
...
if [[ -n "${restart}" ]]; then
    ...
    (
        cd "${RUN_DIR}"
        "${WARPX_EXE}" inputs_scale_pilot_seeded "amr.restart=${restart_rel}" 2>&1 | tee -a run.log
    )
else
    echo "No checkpoint found; starting from t=0"
    (
        cd "${RUN_DIR}"
        "${WARPX_EXE}" inputs_scale_pilot_seeded 2>&1 | tee run.log
    )
fi
